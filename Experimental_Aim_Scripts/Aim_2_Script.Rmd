...
# Plot the taxa that are indicators

df_of_taxa = prune_taxa(pyloseq_object, isa_filtered) %>% psmelt()
...
filtered_stat <- statistical_table |>
  filter(`diff_robust_groupPMS` == "TRUE" )

tax_names <- data.frame(ps_ancomb2@tax_table) 


final_data <- filtered_stat |>
  mutate(names = case_when(
         taxon == "d52a0395431a1912fd52bda732cd2784" ~ "Varibaculum cambriense",
         taxon == "a2149ca6e80cc8bb6b1abda0cce4631a" ~ "Blautia fusiformis"))
...
final_data |>
  ggplot(aes(names, lfc_groupPMS, fill = taxon)) + 
  geom_col() +
  coord_flip() +
